...
async def post_data(url: str, data: dict, headers: dict): async with aiohttp.ClientSession() as session: async with session.post(url, json=data, headers=headers) as resp: return await resp.json()

@app.on_message(filters.command("upscale")) async def upscale_image(_, message: Message): if not YOUR_IMGGEN_API_KEY: return await message.reply_text("🚫 Missing API key.")
...
@app.on_message(filters.command("getdraw")) async def draw_image(_, message: Message): if not YOUR_IMGGEN_API_KEY: return await message.reply_text("🚫 API key is missing.")
